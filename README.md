...
_Schemat_ comes with an internal **data store** that combines NoSQL flexibility with schema awareness. This distributed, object-oriented database uses pure JavaScript for all queries and data mutations, eliminating the need for SQL and custom query languages. The data store supports advanced capabilities: data indexes, object versioning, schema evolution, polymorphic schemas. Web objects are organized into **categories**: schema-defining, class-like structures that specify their valid properties and behavior. _Schemat_ allows web objects to inherit properties through prototype-based inheritance, so programmers can create flexible, reusable hierarchies of objects and categories, that evolve over time. The data store itself is implemented with web objects, demonstrating the platform's self-contained architecture where even core system components follow the same object model as application-level entities.

By introducing [isomorphic](https://en.wikipedia.org/wiki/Isomorphic_JavaScript), network-aware (network-native) objects, _Schemat_ transforms the traditional [OOP](https://en.wikipedia.org/wiki/Object-oriented_programming) paradigm and addresses the distributed nature of modern web applications. Unlike conventional network-agnostic objects that implement only fragments of web functionality tied to an isolated environment; web objects unify the client-side _presentation_ layer, server-side _logic_, and _data model_, all into a single, cohesive entity. This eliminates the fragmentation occuring when separate objects are created and maintained in each execution context, and fosters better modularity and maintainability of application codebases.
As such, _Schemat_ stands as a pioneering software platform that elevates [Object-Oriented Programming](https://developer.mozilla.org/en-US/docs/Learn_web_development/Extensions/Advanced_JavaScript_objects/Object-oriented_programming) (OOP) to the next level, bringing the full power of the object-oriented approach to the Web.

_Schemat_ is implemented in **JavaScript** and **Node.js**, which is a deliberate choice, with JavaScript being the only programming language capable of native execution across all web environments: browsers, servers, data nodes. This unique cross-environment capability is essential for _Schemat_'s vision of creating a single, homogenous runtime environment for application objects, one that spans the entire compute cluster (edge nodes vs data nodes), and the entire network (clients vs servers). <!-- enabling truly isomorphic, web-aware objects. The following list of features demonstrates how _Schemat_ realizes this vision. -->
