...
**Schemat** is an object-oriented platform for building scalable internet applications composed of isomorphic **web objects** that can easily move over the network and execute in any local environment (on client, server, or data node). 
Web objects live _on the Web_, not any single machine. They perform network communication internally via object-to-itself messaging (talking to their own remote copies), thus freeing programmers from the burden of designing custom AJAX / RPC calls and protocols. This simplifies application design and enables programmers to focus on creating valuable features instead of wrestling with complexities of client-server and intra-cluster communication.

_Schemat_ comes with an internal object-oriented **data store** that combines NoSQL flexibility with schema awareness. This distributed database uses pure JavaScript for all queries, eliminating the need for SQL and custom query languages. The data store has advanced capabilities: data indexes, object versioning, schema evolution, polymorphic schemas, data replication (TODO). Web objects are organized into **categories**: schema-defining, class-like structures that specify valid properties and behavior of objects, and are stored in the database like regular objects. _Schemat_ allows web objects to inherit properties through prototype-based inheritance, so programmers can create flexible, reusable hierarchies that evolve over time. The database itself is implemented with web objects, demonstrating the platform's self-contained architecture where even core system components follow the same object model as application-level entities.

By introducing [isomorphic](https://en.wikipedia.org/wiki/Isomorphic_JavaScript), network-aware (network-native) objects, _Schemat_ transforms the traditional [OOP](https://en.wikipedia.org/wiki/Object-oriented_programming) paradigm to address the distributed nature of modern web applications. Unlike conventional network-agnostic objects that implement only fragments of web functionality tied to an isolated environment; web objects unify the client-side _presentation_ layer, server-side _logic_, and _data model_, all into a single, cohesive entity. This eliminates the need for separate object architectures to be established in each environment, and fosters better modularity and maintainability of the application codebase.

_Schemat_ is implemented in **JavaScript** and **Node.js**, which is a deliberate choice, with JavaScript being the only programming language capable of native execution across all web environments: browsers, servers, data nodes. This unique cross-environment capability is essential for _Schemat_'s vision of creating a homogenous runtime environment, one that spans the entire compute cluster (edge nodes vs data nodes), and the entire network (clients vs servers). <!-- enabling truly isomorphic, web-aware objects. The following list of features demonstrates how _Schemat_ realizes this vision. -->
_Schemat_ stands as a pioneering software platform that elevates [Object-Oriented Programming](https://developer.mozilla.org/en-US/docs/Learn_web_development/Extensions/Advanced_JavaScript_objects/Object-oriented_programming) (OOP) to the next level, bringing the full power of the object-oriented approach to the Web.


### Web Object

Features of web objects:
...
- Web objects of selected categories expose another special endpoint, **::admin**, that provides a higher-level administrative interface for performing category-specific actions, like browsing the URL structure of an Application, adding indexes in a Database etc. Like ordinary methods and properties, the implementation of endpoints may be overridden in subcategories and child objects, so, for instance, the default administrative UI can be replaced with a custom one.
...
- **Multiple inheritance** for categories and objects: Schemat-managed properties and the schema definition 
  ...
  The inheritance is **prototypical**, like in plain JavaScript, so it can be applied not only 
...
- **Network-awareness** of application objects, which provide not only ordinary methods for local execution, 
