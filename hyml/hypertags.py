...
*** NEW SYNTAX: "body" attribute of a hypertag declared explicitly as '.' (dot) or '.NAME',
...
*** NEW SYNTAX: braces {{ ... }} as an alternative form of injecting expressions into markup
>>> render("<a>{{ 10 }}</a>")
'<a>10</a>'
...
class HyML_Tree(BaseTree):
    ...
    _ignore_  = "space ws lt gt slash eval comment html_comment lbrace rbrace".split() + parser.noparse_names
    _reduce_  = "def_id literal subexpr slice subscript trailer atom kwattr kwarg xbody_var value1 value2" \
                " tag tag_namecore tag_core tag_name tag_name_start tag_name_end markup value_attr_common"
    _compact_ = "factor term arith_expr concat_expr shift_expr and_expr xor_expr or_expr pipeline comparison" \
