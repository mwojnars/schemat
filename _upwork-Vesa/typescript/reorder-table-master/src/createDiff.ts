import { CatalogState, ROOT_ID } from "./catalogDataContainer";

const EMPTY_LIST = [];
export const getDiff = (
    [hierarchy, valueEntries]: CatalogState,
    [otherHierarchy, otherValueEntries]: CatalogState,
    removedEntries: Set<number>
) => {
    const hierarchyEntries = Array.from(otherHierarchy.entries());
    const resolvePath = (id: number) => {
        if (id === ROOT_ID) {
            return "/";
        } else {
            const parentId =
            ...
            let currentPath: string = valueEntries.get(id)?.[0] ?? null;
    ...
    const hierarchyDiff: Map<string, string[]> = new Map();
    const valueEntryDiff: Map<string, [string, string]> = new Map();

    const allHierarchyEntries: number[] = Array.from(
        new Set<number>(
            Array.from(hierarchy.keys()).concat(
                Array.from(otherHierarchy.keys())
            )
        ).values()
    );
    ...
    const allValueEntries = new Set<number>(
    ...
    const mergedDiff: Map<
        string,
        { hierarchyDiff?: string[]; valueDiff?: [string, string] }
    > = new Map();
