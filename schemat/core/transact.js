    ...
    */
    ...
    get_mutable(obj) {
        ...
        if (existing?.__meta.edits.length) return existing
            ...
            // else throw new Error(`unsaved raw edits exist for ${obj} and the dummy object cannot be edited further`)
            // // never return dummy objects created in stage_edits(), they have no __data and cannot be used for further edits
    ...
    async _save_edited(objects, opts) {
        ...
        for (let obj of objects)
            ...
            else this._staging.delete(obj)                  // remove permanently if a dummy object (no __data)
