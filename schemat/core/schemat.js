...
export class Schemat {
    ...
    constructor(config) {
        ...
        this.config = config
        this.app_id = config.app?.id || config.app || config.app_id || undefined
    ...
    get_object(id, {version = null} = {}) {
        ...
        if (id <= 0 || isNaN(id))
            throw new Error(`object ID must be a positive number, got ${id}`)
    ...
    //    Direct calls execute code on the server via eval, while property access forwards to app.act.XYZ().
    //  */
    // remote = new Proxy(
