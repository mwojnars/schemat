...
export class Loader {
    ...
    async import_module(path, referrer) {
        ...
        let source = await schemat.site.route_internal(path + '::text')
    ...
    async _import_synthetic(path) {
        ...
        let module  = new vm.SyntheticModule(
            ...
            {context, identifier: Loader.DOMAIN_LOCAL + path}
        )
        await module.link(() => {})
    ...
    async _parse_module(source, path) {
        ...
        let linker = async (specifier, ref, extra) => (await this.import_module(specifier, ref)).__vmModule__    //print(specifier, ref) ||
