...
import {MASTER, LocalAtlas} from "./atlas.js";
...
export class Node extends Agent {
    ...
    async rpc_exec(message) {
        ...
        if (!frame) throw new Error(`[${id}].${role || AgentRole.GENERIC} not found on this process to execute RPC message ${JSON.stringify(message)}`)

        return frame.exec(cmd, args, ctx, tx, (out, err) => RPC_Response.create(out, err))
    }

    _find_node(worker, id, role) {
        /* On master, for request routing, find a node where agent (id, role) is deployed. The current node has a priority:
